...
namespace ApiBasic.Shared.ExceptionFiles
{
    public static class HandleFiles
    {
        public static async Task<string> UploadFile(IFormFile _IFormFile)
        {
            string FileName = "";
            try
            {
        ...
        public static async Task<(byte[], string, string)> DownloadFile(string FileName)
        { 
            try
            {
                var _GetFilePath = Common.GetFilePath(FileName);
